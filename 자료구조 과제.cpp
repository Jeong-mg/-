...
void insert_mult(ListType* plist3, int coef, int expon) {
	...
	void poly_multiply(ListType * plist1, ListType * plist2, ListType * plist3)
	{
		...
		while (a != NULL) {
			...
			while (b != NULL) {
				...
				insert_mult(plist3, a->new_coef * b->coef, a->expon + b->expon);
	...
	polynomial = 6^6 + 4^4 + -9^4 + 3^3 + -6^2 + 2^1 +

	C:\Users\USER\source\repos\Project4\x64\Debug\Project4.exe(프로세스 18244개)이(가) 종료되었습니다(코드: 0개).
